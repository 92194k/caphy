...
class BilingualVoice:
    """Runs several Vosk models on ONE microphone at the same time. Whichever
    model recognizes a phrase yields it - so English and Tagalog both work
    without the user switching anything. Falls back to a single model if only
    one is given."""
    def __init__(self, model_paths, sample_rate=16000):
        from vosk import Model, KaldiRecognizer
        self.sample_rate = sample_rate
        self.recs = []
        for path in model_paths:
            model = Model(path)
            self.recs.append(KaldiRecognizer(model, sample_rate))
        self.q = queue.Queue()
        self._tts = None
        try:
            import pyttsx3
